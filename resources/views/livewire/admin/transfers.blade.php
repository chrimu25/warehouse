        ...
        <x-table>
            ...
            <x-table.row>
                  ...
                  <div class="flex items-center sm:flex-column sm:items-start">
                    <div class="ml-4">
                        ...
                        <div class="text-gray-500">
                            ...
                            <div>Date:<span class="ml-1">{{$item->created_at->format('Y-m-d')}}</span></div> 
                        </div>
                    </div>
                  </div>
                </x-table.cell>
                <x-table.cell data-label="Owner">
                  <div class="flex items-center sm:flex-column sm:items-start">
                    <div class="ml-4">
                        <span>{{$item->owner1->exists()?$item->owner1->name:''}}</span>
                        <div class="text-gray-500">
                            <div class="my-1">phone:<span><a href="tel:{{$item->owner1->exists()?$item->owner1->phone:''}}" class="mr-2">{{$item->owner1->exists()?$item->owner1->phone:''}}</a></span></div> 
                            <div>Email:<span><a href="mailto:{{$item->owner1->exists()?$item->owner1->email:''}}">{{$item->owner1->exists()?$item->owner1->email:''}}</a></span></div> 
                        </div>
                    </div>
                  </div>
                </x-table.cell>
                <x-table.cell data-label="From">
                  <div class="flex items-center sm:flex-column sm:items-start">
                    <div class="ml-4">
                        ...
                        <div class="text-gray-500">
                            <div class="my-1">Slot:<span>{{$item->product->exists()?$item->product->slot->name:''}}</span></div> 
                            <div>Store Until:<span>{{$item->product->exists()?$item->product->until->format('Y-m-d'):''}}</span></div> 
                        </div>
                    </div>
                  </div>
                </x-table.cell>
                <x-table.cell data-label="To">
                  <div class="flex items-center sm:flex-column sm:items-start">
                    <div class="ml-4">
                        ...
                        <div class="text-gray-500">
                            <div class="my-1">Slot:<span>{{$item->slot->exists()?$item->slot->name:''}}</span></div> 
                            <div>Store Until:<span>{{$item->until->format('Y-m-d')}}</span></div> 
                        </div>
                    </div>
                  </div>
                </x-table.cell>
